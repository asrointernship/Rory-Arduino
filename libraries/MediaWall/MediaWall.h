...
//#include "MediaWall_Commands.h" - R9 removed
...
const byte MediaWall_CURSOR_PATTERN_SELECT=0xA0; //cursor patter select command prefix or with desired size-1.
const byte MediaWall_DISPLAY_MODE=0x90; 
const byte MediaWall_MODE_SET=0x80;
const byte MediaWall_SET_CURSOR_POINTER=0x21;
const byte MediaWall_SET_OFFSET_REGISTER=0x22;
const byte MediaWall_SET_ADDRESS_POINTER=0x24;
const byte MediaWall_SET_TEXT_HOME_ADDRESS=0x40;
const byte MediaWall_SET_TEXT_AREA=0x41;
const byte MediaWall_SET_GRAPHIC_HOME_ADDRESS=0x42;
const byte MediaWall_SET_GRAPHIC_AREA=0x43;
const byte MediaWall_SET_DATA_AUTO_WRITE=0xB0;
const byte MediaWall_SET_DATA_AUTO_READ=0xB1;
const byte MediaWall_AUTO_RESET=0xB2;

const byte MediaWall_DATA_WRITE_AND_INCREMENT=0xC0;
const byte MediaWall_DATA_READ_AND_INCREMENT=0xC1;
const byte MediaWall_DATA_WRITE_AND_DECREMENT=0xC2;
const byte MediaWall_DATA_READ_AND_DECREMENT=0xC3;
const byte MediaWall_DATA_WRITE_AND_NONVARIALBE=0xC4;
const byte MediaWall_DATA_READ_AND_NONVARIABLE=0xC5;

const byte MediaWall_SCREEN_PEEK=0xE0;
const byte MediaWall_SCREEN_COPY=0xE8;

class MediaWall{
public:
  MediaWall(int pixHoriz,int pixVert,int fontWidth, int sizeMem, byte ce); //Class
  ...
  void glcd_print1_P(unsigned char, unsigned char, const char*, const FONT_DEF*, unsigned char);
    void glcd_print1_halfLetter(unsigned char, unsigned char, const char*, const FONT_DEF*, unsigned char);
    void glcd_print1_textbox(unsigned char,unsigned char,unsigned char,unsigned char,const char*,const struct FONT_DEF*,unsigned char,unsigned char);
    void glcd_print1_wrap(unsigned char*, unsigned char*, const char*, const FONT_DEF*, unsigned char);
  void glcd_print1_center(unsigned char, const char*, const FONT_DEF*, unsigned char);
    void glcd_print1_right(unsigned char, const char*, const FONT_DEF*, unsigned char, unsigned char);
    void glcd_print1_half_right(unsigned char, const char*, const FONT_DEF*, unsigned char);
...
private:
  ...
  byte _sizeMem;  //memory location for CGRAM (2kb required, 64kb max memory controlled by MediaWall, 32 possible locations)
  byte GLCD_CE;
